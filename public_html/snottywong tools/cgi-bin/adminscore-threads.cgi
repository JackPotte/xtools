...
def main():
    ...
    <div style="width:875px;">"""
        ...
        #score += aivrppafd(sql, name)  <--Takes too long, threads?
        #debug("aivrppafd")
        #score += patrols(sql, name)  <--Takes too long, might try to fix later
        if not currentrfa:
...
def patrols(sql, name): #Query takes too long, skipping for now
    pass
